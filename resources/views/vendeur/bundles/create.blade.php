...
<div class="pb-20">
    <div class="mb-8">
        <h1 class="font-serif text-[32px] text-[#0a0a0a] mb-2">Nouveau Bundle</h1>
        <p class="text-[13px] text-[#a0a09a]">Créez une offre groupée de 2+ produits</p>
    </div>

    <form action="{{ route('vendeur.bundles.store') }}" method="POST" class="max-w-2xl space-y-4">
        @csrf
        ...
        <div class="bg-white border border-[#e0e0dc] rounded-xl p-5">
            ...
            <input type="text" id="nom" name="nom" required
                   value="{{ old('nom') }}" placeholder="Ex: Pack Gaming Ultimate"
        ...
        <div class="bg-white border border-[#e0e0dc] rounded-xl p-5">
            ...
            <textarea id="description" name="description" rows="3"
                      ...
                      class="w-full px-3.5 py-3 text-[13px] text-[#0a0a0a] bg-[#f7f7f5] border border-[#e0e0dc] rounded-lg focus:outline-none focus:border-[#0a0a0a] focus:bg-white transition-colors resize-none">{{ old('description') }}</textarea>
        </div>
        ...
        <div class="bg-white border border-[#e0e0dc] rounded-xl p-5">
            ...
            <div id="produits-container" class="space-y-3">
                <div class="produit-row flex gap-2 items-end">
                    ...
                    <input type="number" name="quantites[0]" min="1" value="1" placeholder="Qté" class="w-16 px-2 py-2 text-[12px] border border-[#e0e0dc] rounded-lg">
        ...
        <div class="grid grid-cols-2 gap-4">
            ...
            <div class="bg-white border border-[#e0e0dc] rounded-xl p-5">
                ...
                <input type="number" id="quantite_disponible" name="quantite_disponible" min="1"
                       value="{{ old('quantite_disponible') }}" placeholder="Illimité"
                       class="w-full px-3.5 py-3 text-[13px] text-[#0a0a0a] bg-[#f7f7f5] border border-[#e0e0dc] rounded-lg focus:outline-none focus:border-[#0a0a0a] focus:bg-white transition-colors">
            </div>
        </div>
        ...
        <div class="flex gap-3">
            <a href="{{ route('vendeur.bundles.index') }}" class="flex-1 px-4 py-3 border border-[#e0e0dc] rounded-lg text-[13px] font-medium text-[#666660] hover:border-[#0a0a0a] transition-colors text-center">
                Annuler
            </a>
...
document.getElementById('add-produit-btn').addEventListener('click', (e) => {
    ...
    row.innerHTML = `
        ...
        <input type="number" name="quantites[${produitCount}]" min="1" value="1" placeholder="Qté" class="w-16 px-2 py-2 text-[12px] border border-[#e0e0dc] rounded-lg">
        <button type="button" class="px-2 py-2 text-red-600 hover:bg-red-50 rounded remove-produit-btn">✕</button>
    `;
