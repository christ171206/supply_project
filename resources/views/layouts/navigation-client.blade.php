    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div x-show="open" class="md:hidden border-t border-slate-700/50 py-4 space-y-2">
            ...
            @endauth
                                ...
                                <!-- Mes Commandes -->
                                <a href="{{ route('client.commandes') }}" class="block px-4 py-2 text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition">
                                    📋 Mes Commandes
                                </a>

                                <!-- Messages -->
                                <a href="{{ route('client.messages') }}" class="block px-4 py-2 text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition">
                                    💬 Messages
                                </a>

                                <!-- Profil -->
                                <a href="{{ route('client.profil') }}" class="block px-4 py-2 text-gray-700 hover:bg-blue-50 hover:text-blue-600 transition">
                                    👤 Mon Profil
                                </a>
                            @endif
            ...
            <div class="md:hidden flex items-center gap-4">
                <!-- Panier Mobile -->
                <a href="{{ route('panier.index') }}" class="relative text-gray-700 hover:text-blue-600">
                    🛒
                    <span id="cart-badge-mobile" class="absolute -top-2 -right-2 bg-red-600 text-white text-xs rounded-full w-4 h-4 flex items-center justify-center @if(!auth()->check() || !auth()->user()->panier || auth()->user()->panier->items->count() === 0) hidden @endif">
                        {{ auth()->check() && auth()->user()->panier ? auth()->user()->panier->items->count() : '0' }}
                    </span>
                </a>
        ...
        <div x-show="open" class="md:hidden border-t border-gray-200 py-4 space-y-2">
            <a href="{{ route('accueil') }}" class="block px-4 py-2 text-gray-700 hover:bg-blue-50 rounded transition">
                🏠 Accueil
            </a>

            <a href="{{ route('produits.catalogue') }}" class="block px-4 py-2 text-gray-700 hover:bg-blue-50 rounded transition">
                📦 Catalogue
            </a>

            @auth
                @if(Auth::user()->role === 'vendeur')
                    <a href="{{ route('vendeur.dashboard') }}" class="block px-4 py-2 text-gray-700 hover:bg-purple-50 rounded transition font-bold">
                        🏪 Espace Vendeur
                    </a>
                    <a href="{{ route('client.dashboard') }}" class="block px-4 py-2 text-gray-700 hover:bg-blue-50 rounded transition">
                        📊 Mon Tableau de Bord
                    </a>
                    <a href="{{ route('client.commandes') }}" class="block px-4 py-2 text-gray-700 hover:bg-blue-50 rounded transition">
                        📋 Mes Commandes
                    </a>
                    <a href="{{ route('client.profil') }}" class="block px-4 py-2 text-gray-700 hover:bg-blue-50 rounded transition">
                        👤 Profil
                    </a>
                @else
                    <a href="{{ route('client.dashboard') }}" class="block px-4 py-2 text-gray-700 hover:bg-blue-50 rounded transition">
                    ...
                    <a href="{{ route('client.messages') }}" class="block px-4 py-2 text-gray-700 hover:bg-blue-50 rounded transition">
                        💬 Messages
                    </a>
                    <a href="{{ route('client.profil') }}" class="block px-4 py-2 text-gray-700 hover:bg-blue-50 rounded transition">
                        👤 Profil
                ...
                @endif
                <div class="px-4 py-2">
                    <span class="inline-block text-xs px-3 py-1 rounded-full {{ Auth::user()->role === 'vendeur' ? 'bg-purple-100 text-purple-800' : 'bg-blue-100 text-blue-800' }}">
                        {{ Auth::user()->role === 'vendeur' ? '🏪 Vendeur' : '🛒 Client' }}
                    </span>
                </div>
                <form method="POST" action="{{ route('logout') }}">
                    @csrf
            ...
            @else
                <a href="{{ route('login') }}" class="block px-4 py-2 text-gray-700 hover:bg-blue-50 rounded transition">
                    🔐 Connexion
                </a>
                <a href="{{ route('register') }}" class="block px-4 py-2 bg-blue-600 text-white rounded transition">
                    ✍️ S'inscrire
                </a>
            @endauth
        </div>
