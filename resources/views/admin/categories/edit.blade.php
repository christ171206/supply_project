...
@section('title', 'Éditer catégorie — Supply Admin')
        ...
        <div class="max-w-xl w-full">
            ...
            <div class="bg-white border border-[#e0e0dc] rounded-xl overflow-hidden">
                <div class="px-6 py-5 border-b border-[#efefed]">
                    <span class="text-[13px] font-medium text-[#0a0a0a]">Modifier les informations</span>
                </div>
                ...
                <form action="{{ route('admin.categories.update', $category) }}" method="POST" enctype="multipart/form-data">
                    ...
                    <div class="px-6 py-5 space-y-5">
                        ...
                        <div>
                            ...
                            @if($category->image)
                                <div class="mb-3">
                                    <div class="relative w-full h-24 bg-[#f7f7f5] rounded-lg border border-[#e0e0dc] overflow-hidden flex items-center justify-center">
                                        <img src="{{ asset('storage/' . $category->image) }}"
                                             alt="{{ $category->nom }}"
                                             class="h-full object-contain">
                                    </div>
                                    <p class="text-[11px] text-[#a0a09a] mt-1.5">Image actuelle</p>
                                </div>
                            @endif
                        ...
                        <div>
                            ...
                            <textarea name="description" id="description" rows="4"
                                      ...
                                      class="w-full bg-[#f7f7f5] border border-[#e0e0dc] rounded-lg px-3 py-2.5 text-[13px] text-[#0a0a0a]
                                             ...
                                             @error('description') border-[#f87171] bg-[#fef2f2] @enderror">{{ old('description', $category->description) }}</textarea>
                            @error('description')
                                <p class="text-[11px] text-[#dc2626] mt-1">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div class="flex items-center gap-3">
                            <button type="button" id="toggle-active"
                                    ...
                                    class="relative w-9 h-5 rounded-full transition-colors duration-200 {{ old('is_active', $category->is_active) ? 'bg-[#0a0a0a]' : 'bg-[#e0e0dc]' }}"
                                    data-active="{{ old('is_active', $category->is_active) ? '1' : '0' }}">
                                <span id="toggle-thumb"
                                      class="absolute top-0.5 w-4 h-4 bg-white rounded-full transition-transform duration-200
                                             {{ old('is_active', $category->is_active) ? 'translate-x-4' : 'translate-x-0.5' }}"></span>
                            </button>
                            <input type="hidden" name="is_active" id="is_active-input"
                                   value="{{ old('is_active', $category->is_active) ? '1' : '0' }}">
